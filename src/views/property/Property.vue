<template>
  <div class="container">
    <span v-if="totalPropertiesCount != 0" class="display-4">Total properties: {{totalPropertiesCount}}</span>
    ...
    <div v-if = "properties.length == 0" class="alert alert-info">
      No properties found, please <router-link to="/createproperty">create property</router-link>  
    </div>
    <div class="row">
        <div class="col-12" v-for="property in properties" :key = "property.id">
            <div class="card shadow">
              <div class="card-body">
                ...
                <a href="property.id" class="card-link">Detail</a>
...
methods: {
  ...
  deleteProperty(propertyId){  
    this.loading = true;
    PropService.deleteProperty(this.$store.state.auth.user.id, propertyId)
    .then(
      response => {
